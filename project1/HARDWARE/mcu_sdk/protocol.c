...
#include "bluetooth.h"
  ...
  enum battery_state {low,middle,high};
	enum pir_state {pir,none};
...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_TEMPER_ALARM, DP_TYPE_BOOL},
  ...
  {DPID_PIR_TIME, DP_TYPE_ENUM},
	{DPID_LEIDA, DP_TYPE_VALUE},
	{DPID_LEIDA2, DP_TYPE_ENUM},
...
******************************************************************************/
void uart2_blue_send_data(unsigned  char value)
{
...
//  //此代码为平台自动生成，请按照实际数据修改每个可下发可上报函数和只上报函数
    mcu_dp_enum_update(DPID_PIR_STATE,pir); //枚举型数据上报;
    mcu_dp_enum_update(DPID_BATTERY_STATE,high); //枚举型数据上报;
//    mcu_dp_bool_update(DPID_TEMPER_ALARM,当前防拆报警); //BOOL型数据上报;
    mcu_dp_value_update(DPID_BRIGHT_VALUE,50); //VALUE型数据上报;
    mcu_dp_value_update(DPID_TEMP_CURRENT,humiture[0]*10); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUMIDITY_VALUE,humiture[2]); //VALUE型数据上报;
...
static unsigned char leida(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char pir_time;
...
//    }
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_LEIDA2, pir_time);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
void  bluetooth_to_scrawl(void)
{
	bt_uart_service();//blu 串口服务协议
